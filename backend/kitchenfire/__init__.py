...
def create_post_by_row(post_id: int) -> Post:
    with sqlite3.connect("data/fire.db") as db:
        c = db.cursor()
        c.execute("SELECT NumberOfLikes, Rating, Reviews FROM Posts WHERE RecipeId = ?", (post_id,))
        (likes, rating, reviews) = c.fetchone()
        tags = tags_by_recipe_id(post_id)
        ingredients = ingredients_by_recipe_id(post_id)
        recipe_tuple: tuple = c.execute("SELECT RecipeName, Description, Instructions, CookTime, Difficulty, PhotoURL FROM Recipes WHERE RecipeId = ?", (post_id,)).fetchone();
...
def get_recipe_by_id(recipe_id):
    return Response(create_post_by_row(int(recipe_id)).to_json(), content_type="application/json")
