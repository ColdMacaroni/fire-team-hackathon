  ...
  <div class="recipe-view">
    ...
    <div v-else-if="recipe" class="recipe-container">
      <div class="recipe-header">
        <img v-if="recipe.image && !recipe.image.startsWith('#')" :src="recipe.image" alt="Recipe Image"
          class="recipe-image" />
        <div v-else-if="recipe.image && recipe.image.startsWith('#')" class="recipe-image"
          :style="{ backgroundColor: recipe.image }"></div>
      ...
      <div class="recipe-content">
        ...
        <div class="recipe-rating">
          ...
          <button @click="doLike" class="like-button" :class="{ liked: isLiked1 }">
            🔥
          </button>
          <button @click="viewComments" class="like-button" :class="{ liked: isLiked2 }">
            🗨️
          </button>
          <button @click="doDislike" class="like-button" :class="{ liked: isLiked3 }">
            🧯
          </button>
        </div>
        <div class="recipe-tags">
          <p>
            Tags:
            <span v-for="(tag, index) in parsedTags" :key="tag.id || index" class="tag-item">
              {{ tag.name }}{{ index < parsedTags.length - 1 ? ', ' : '' }} </span>
          </p>
        </div>
        <div class="recipe-ingredients">
          <h2>Ingredients</h2>
          <ul>
            <li v-for="ingredient in parsedIngredients" :key="ingredient.id || ingredient.ingredient"
              class="ingredient-item" :class="{ checked: isIngredientChecked(ingredient.ingredient) }"
              @click="toggleIngredient(ingredient.ingredient)">
              <div class="ingredient-icon">
                <FontAwesomeIcon v-if="isIngredientChecked(ingredient.ingredient)" icon="fa-times" class="cross-icon" />
...
.recipe-name {
  ...
  /* Changed from center to allow for multi-line text */
  min-height: 40px;
  /* Increased height for better text wrapping */
  gap: 16px;
  /* Add gap between title and like button */
}
...
.recipe-name .like-button {
  flex-shrink: 0;
  /* Prevent like button from shrinking */
  align-self: center;
  /* Center the like button vertically */
}
...
.recipe-rating {
  ...
  justify-content: space-between;
  width: 98%;
  /* Makes the like button align with the number */
}
