...
<template>
  <div class="large-card" v-if="recipe">
    <img v-if="recipe.image && !recipe.image.startsWith('#')" :src="recipe.image" @click="navigateToRecipe(recipe.id)"
      alt="Image couldn't be loaded." class="recipe-image" />
    <div v-else-if="recipe.image && recipe.image.startsWith('#')" class="recipe-image"
      :style="{ backgroundColor: recipe.image }" @click="navigateToRecipe(recipe.id)"></div>
